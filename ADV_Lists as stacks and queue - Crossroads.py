...
while True:
    ...
    if command == 'END' or crash == 1:
        ...
        while cars:
         if len(car)==0 and free >1 and free <=free_window: # ако колата е излязла и вече сме в свободния прозорец
             break

         elif len(car)==0: # ако колата е излязла и сме в зелено, влиза следващата кола

           car = cars[0]

         elif len(car) > 0: # ако колата не е излязла, продължава да се движи напред
              car = car[1:]

              if green < green_light : # по време на зелено
                 green +=1


              elif green == green_light and free < free_window: # по време на прозореца
                   free +=1


              elif green == green_light and free == free_window and len(car) > 0: # катастрофа
                                               # ако зеленото и прозореца са свършили и колата е още в кръстовището
                     crash = 1
                     break


         if green <= green_light and free <= free_window and len(car) == 0: # ако колата е излязла преди да свърши зеленото и свободния прозорец
                                                                            # намаляваме колите и броим +1 преминала
              cars.pop(0)
              passed +=1
